...
import { loadProjectsIndexFromBackend } from "../utils/runtimeProjects";
import {
  loadActionCatalog,
...
const projectsListUrl = getApiUrl("projects/list");
const projectsIndexLoadUrl = getApiUrl("projects/load?name=projects.json");
const projectsIndexSaveUrl = getApiUrl("projects/save");
...
const projectLoadUrl = (projectName) => {
  ...
  return getApiUrl(`projects/load?name=${encodedName}`);
};
...
const loadProjectsIndex = async () => {
  persistAvailable.value = false;
  const result = await loadProjectsIndexFromBackend({
    fetchIndex: () => fetchJson(projectsIndexLoadUrl),
    readFallback: readStaticProjectsIndex
  });
  persistAvailable.value = result.canPersist;
  return result.data;
};
...
const persistProjectsIndex = async () => {
  ...
  try {
    const response = await fetchJson(projectsIndexSaveUrl, {
      ...
      body: JSON.stringify({ name: "projects.json", data: payload })
    });
    if (!response.ok) {
