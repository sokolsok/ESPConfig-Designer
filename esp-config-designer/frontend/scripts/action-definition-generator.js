...
const bytePayloadField = () => ({
  ...
  lambdaPlaceholder: "return std::vector<uint8_t>{0xA5, 0x5A};"
});
...
const EXACT_GENERATED_ACTIONS = {
  "logger.set_level": singletonAction("logger.set_level", [
    targetField("logger"),
    scalarField("level", "Level", {
      type: "select",
      templatable: false,
  ...
  "dfplayer.set_eq": singletonAction("dfplayer.set_eq", [
    targetField("dfplayer"),
    scalarField("eq_preset", "EQ preset", {
      type: "select",
      required: true,
      options: ["NORMAL", "POP", "ROCK", "JAZZ", "CLASSIC", "BASS"]
    })
  ]),
  "dfplayer.set_device": singletonAction("dfplayer.set_device", [
    targetField("dfplayer"),
    scalarField("device", "Device", {
      type: "select",
      templatable: false,
      required: true,
      options: ["USB", "TF_CARD"]
  ...
  "climate.haier.set_vertical_airflow": singletonAction("climate.haier.set_vertical_airflow", [
    targetField("climate"),
    scalarField("vertical_airflow", "Vertical airflow", {
      type: "select",
      required: true,
      options: ["HEALTH_UP", "MAX_UP", "UP", "CENTER", "DOWN", "HEALTH_DOWN"]
    })
  ]),
  "climate.haier.set_horizontal_airflow": singletonAction("climate.haier.set_horizontal_airflow", [
    targetField("climate"),
    scalarField("horizontal_airflow", "Horizontal airflow", {
      type: "select",
      required: true,
      options: ["MAX_LEFT", "LEFT", "CENTER", "RIGHT", "MAX_RIGHT"]
    })
  ]),
  ...
  "sim800l.send_sms": singletonAction("sim800l.send_sms", [
    ...
    scalarField("recipient", "Recipient", { required: true, placeholder: "+48123456789", lambdaPlaceholder: "return std::string(\"+48123456789\");" }),
  ...
  "servo.write": singletonAction("servo.write", [
    targetField("servo"),
    {
      ...
      required: true,
      lvl: "normal",
      note: "Literal values use percentages from -100% to 100%. Lambda values should return a float from -1.0 to 1.0.",
      placeholder: "50%",
      lambdaPlaceholder: "return 0.5;"
    }
  ]),
  "servo.detach": singletonAction("servo.detach", [targetField("servo")]),
  ...
  "rf_bridge.send_code": singletonAction("rf_bridge.send_code", [
    ...
    scalarField("code", "Code", { type: "number", required: true, templatable: false, placeholder: "123456" })
  ]),
  ...
  "grove_tb6612fng.change_address": singletonAction("grove_tb6612fng.change_address", [
    targetField("grove_tb6612fng", { label: "Grove TB6612FNG" }),
  ...
  "grove_tb6612fng.run": singletonAction("grove_tb6612fng.run", [
    targetField("grove_tb6612fng", { label: "Grove TB6612FNG" }),
    scalarField("channel", "Channel", { type: "number", required: true, placeholder: "0", lambdaPlaceholder: "return 0;" }),
    scalarField("direction", "Direction", { type: "select", templatable: false, required: true, options: ["FORWARD", "BACKWARD"] }),
    scalarField("speed", "Speed", { type: "number", required: true, placeholder: "255", lambdaPlaceholder: "return 255;" })
  ]),
  "grove_tb6612fng.standby": singletonAction("grove_tb6612fng.standby", [
    targetField("grove_tb6612fng", { label: "Grove TB6612FNG" })
  ]),
  "grove_tb6612fng.no_standby": singletonAction("grove_tb6612fng.no_standby", [
    targetField("grove_tb6612fng", { label: "Grove TB6612FNG" })
  ]),
  ...
  "lightwaverf.send_raw": singletonAction("lightwaverf.send_raw", [
    ...
    scalarField("name", "Name", { required: true, templatable: false, placeholder: "lamp" }),
    {
      key: "code",
      label: "Code",
      type: "list",
  ...
  "rtttl.play": singletonAction("rtttl.play", [
    targetField("rtttl", { required: false, label: "RTTTL" }),
    {
      ...
      required: true,
      lvl: "normal",
      note: "If a melody is already playing, stop it first before starting a new one.",
      placeholder: "two_short:d=4,o=5,b=100:16e6,16e6"
    }
  ]),
  "rtttl.stop": singletonAction("rtttl.stop", [targetField("rtttl", { required: false, label: "RTTTL" })]),
  ...
  "lock.template.publish": singletonAction("lock.template.publish", [
    targetField("lock"),
    scalarField("state", "State", { required: true, placeholder: "LOCK_STATE_LOCKED" })
  ]),
...
export const generateActionDefinition = (action) => {
  ...
  if (action.id === "datetime.time.set") {
    return singletonAction(action.id, [targetField("datetime"), scalarField("time", "Time", { required: true, placeholder: "14:30:00", templatable: false })]);
  }
...
export const validateActionCatalog = (catalog) => {
  if (!Array.isArray(catalog?.actions)) {
    throw new Error("Invalid action catalog:\n- catalog.actions must be an array");
  }
  if (catalog.actions.length === 0) {
    throw new Error("Invalid action catalog:\n- catalog.actions must not be empty");
  }
  ...
  catalog.actions.forEach((action, index) => {
    ...
    const location = id || `actions[${index}]`;

    if (!/^[a-z0-9_]+(?:\.[a-z0-9_]+)*$/.test(id)) {
      errors.push(`${location}: id must be a dot-separated lowercase identifier`);
      return;
    }

    if (actionIds.has(id)) {
      errors.push(`${id}: duplicate action id`);
    }
    actionIds.add(id);

    const expectedSchemaUrl = `actions/${actionPathFromId(id)}`;
    if (action.schemaUrl !== expectedSchemaUrl) {
      errors.push(`${id}: schemaUrl must be ${JSON.stringify(expectedSchemaUrl)}, received ${JSON.stringify(action.schemaUrl)}`);
    }
  });
...
const validateFieldType = (field, location, errors) => {
  if (!SUPPORTED_FIELD_TYPES.has(field.type)) {
    errors.push(`${location}: unsupported field type ${JSON.stringify(field.type)}`);
  }
};

const validateFields = (fields, location, errors) => {
  ...
  fields.forEach((field, index) => {
    ...
    if (!isObject(field)) {
      errors.push(`${fieldLocation} must be an object`);
      return;
    }

    if (typeof field.key !== "string" || field.key.trim() === "") {
      errors.push(`${fieldLocation}: field key must be a non-empty string`);
    } else if (fieldKeys.has(field.key)) {
      errors.push(`${location}: duplicate field key ${JSON.stringify(field.key)}`);
    } else {
      fieldKeys.add(field.key);
    }

    validateFieldType(field, fieldLocation, errors);
    if (field.fields !== undefined) {
      validateFields(field.fields, `${fieldLocation}.fields`, errors);
    }
    if (field.item !== undefined) {
      if (!isObject(field.item)) {
        errors.push(`${fieldLocation}.item must be an object`);
      } else {
        validateFieldType(field.item, `${fieldLocation}.item`, errors);
        if (field.item.fields !== undefined) {
          validateFields(field.item.fields, `${fieldLocation}.item.fields`, errors);
        }
      }
    }
  });
...
export const validateActionDefinitions = (definitions) => {
  if (!Array.isArray(definitions)) {
    throw new Error("Invalid generated action definitions:\n- definitions must be an array");
  }

  const errors = [];
  const schemaPaths = new Set();
  const caseFoldedSchemaPaths = new Map();

  definitions.forEach((entry, index) => {
    const location = typeof entry?.id === "string" && entry.id ? entry.id : `definitions[${index}]`;
    const schemaPath = typeof entry?.schemaPath === "string" ? entry.schemaPath : "";

    if (!schemaPath) {
      errors.push(`${location}: schema path must be a non-empty string`);
    } else {
      if (schemaPaths.has(schemaPath)) {
        errors.push(`${location}: duplicate schema path ${JSON.stringify(schemaPath)}`);
      }
      schemaPaths.add(schemaPath);

      const foldedPath = schemaPath.toLowerCase();
      const existingPath = caseFoldedSchemaPaths.get(foldedPath);
      if (existingPath && existingPath !== schemaPath) {
        errors.push(`${location}: schema path case collision between ${JSON.stringify(existingPath)} and ${JSON.stringify(schemaPath)}`);
      } else {
        caseFoldedSchemaPaths.set(foldedPath, schemaPath);
      }
    }

    if (!isObject(entry?.definition)) {
      errors.push(`${location}: definition must be an object`);
      return;
    }
    if (entry.definition.id !== entry.id) {
      errors.push(`${location}: definition id must equal catalog action id`);
    }
    validateFields(entry.definition.fields, `${location}.fields`, errors);
  });
...
export const buildActionDefinitions = (catalog) => {
  const actions = validateActionCatalog(catalog);
  const definitions = actions.map((action) => {
  ...
  return validateActionDefinitions(definitions);
};
...
const listJsonFiles = (rootDir) => {
  ...
  while (stack.length) {
    ...
    fs.readdirSync(current, { withFileTypes: true }).forEach((entry) => {
      ...
      if (entry.isFile() && nextPath.toLowerCase().endsWith(".json")) {
        files.push(nextPath);
      }
    });
  }
...
const serializeDefinition = (definition) => `${JSON.stringify(definition, null, 2)}\n`;

const loadActionDefinitions = (catalogPath) => {
  const catalog = JSON.parse(fs.readFileSync(catalogPath, "utf8"));
  return buildActionDefinitions(catalog);
};

const toSchemaPath = (outputDir, filePath) => path.relative(outputDir, filePath).split(path.sep).join("/");

const getExpectedFiles = (definitions) => new Map(
  definitions.map(({ schemaPath, definition }) => [
    schemaPath,
    { schemaPath, content: serializeDefinition(definition) }
  ])
);

const getActualFiles = (outputDir) => listJsonFiles(outputDir).map((filePath) => {
  const schemaPath = toSchemaPath(outputDir, filePath);
  return { schemaPath, foldedPath: schemaPath.toLowerCase(), filePath };
});

let caseRenameSequence = 0;

const renameWithCanonicalCase = (existingPath, canonicalPath) => {
  const parentDir = path.dirname(existingPath);
  let temporaryPath;
  do {
    caseRenameSequence += 1;
    temporaryPath = path.join(parentDir, `.action-definition-case-${process.pid}-${caseRenameSequence}`);
  } while (fs.existsSync(temporaryPath));

  fs.renameSync(existingPath, temporaryPath);
  try {
    fs.renameSync(temporaryPath, canonicalPath);
  } catch (error) {
    fs.renameSync(temporaryPath, existingPath);
    throw error;
  }
};
...
const canonicalizePath = (outputDir, schemaPath) => {
  ...
  segments.forEach((segment, index) => {
    ...
    const existingName = entries.find((entry) => entry === segment)
      || entries.find((entry) => entry.toLowerCase() === segment.toLowerCase());

    if (existingName && existingName !== segment) {
      renameWithCanonicalCase(path.join(currentDir, existingName), path.join(currentDir, segment));
    }
...
export const checkActionDefinitions = ({ catalogPath, outputDir }) => {
  const definitions = loadActionDefinitions(catalogPath);
  const expectedFiles = getExpectedFiles(definitions);
  const actualFiles = getActualFiles(outputDir);
  const actualFilesByPath = new Map(actualFiles.map((file) => [file.schemaPath, file]));
  const actualFilesByFoldedPath = new Map();
  actualFiles.forEach((file) => {
    const matches = actualFilesByFoldedPath.get(file.foldedPath) || [];
    matches.push(file);
    actualFilesByFoldedPath.set(file.foldedPath, matches);
  });
  const consumedFiles = new Set();
  const drift = [];

  expectedFiles.forEach((expectedFile, schemaPath) => {
    const actualFile = actualFilesByPath.get(schemaPath);
    if (!actualFile) {
      const caseMatch = (actualFilesByFoldedPath.get(schemaPath.toLowerCase()) || [])
        .find((file) => !consumedFiles.has(file.filePath));
      if (caseMatch) {
        consumedFiles.add(caseMatch.filePath);
        drift.push({ type: "case", path: schemaPath, actualPath: caseMatch.schemaPath });
        return;
      }
      drift.push({ type: "missing", path: expectedFile.schemaPath });
      return;
    }
    consumedFiles.add(actualFile.filePath);
    if (fs.readFileSync(actualFile.filePath, "utf8") !== expectedFile.content) {
      drift.push({ type: "changed", path: expectedFile.schemaPath });
    }
  });

  actualFiles.forEach((actualFile) => {
    if (!consumedFiles.has(actualFile.filePath)) {
      drift.push({ type: "stale", path: actualFile.schemaPath });
    }
  });
  ...
  return { definitions, drift };
};

export const writeActionDefinitions = ({ catalogPath, outputDir, prune = false }) => {
  const definitions = loadActionDefinitions(catalogPath);
  const expectedFiles = getExpectedFiles(definitions);
  ...
  const staleFiles = getActualFiles(outputDir)
    .filter((actualFile) => !expectedFiles.has(actualFile.schemaPath))
    .sort((left, right) => left.schemaPath < right.schemaPath ? -1 : left.schemaPath > right.schemaPath ? 1 : 0);

  if (prune) {
    staleFiles.forEach((staleFile) => fs.rmSync(staleFile.filePath));
  }

  return {
    definitions,
    stale: prune ? [] : staleFiles.map((file) => file.schemaPath),
    pruned: prune ? staleFiles.map((file) => file.schemaPath) : []
  };
};
