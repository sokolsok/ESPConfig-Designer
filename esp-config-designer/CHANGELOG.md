...
* Published ESPConfig Designer as an [unsigned public Windows 11 x64 release](https://github.com/sokolsok/ESPConfig-Designer/releases/tag/windows-1.4.0-unsigned.1) with embedded Python, ESPHome, PlatformIO, and MinGit, backed by the shared application sources and isolated workspace/app-data storage. Windows may display `Unknown Publisher` because the installer is not Authenticode-signed.
* Verified hosted installation and smoke plus clean-machine firmware online, reboot-resume, offline replay, and cancel on Windows 11 Home 25H2 x64 build `26200.8973`. Full Windows 10 support is not declared.
* Kept Windows updates manual with no auto-updater; installer integrity is published through SHA-256 checksums on the Release page.
...
* Added warm second-launch handling that preserves one Desktop backend; restoring or focusing an already minimized window remains a [known Windows limitation](https://github.com/sokolsok/ESPConfig-Designer/blob/93bc7858d3855fded3e610b6d63dc9b2bf7e16f6/KNOWN_ISSUES.md#windows-desktop-140).
