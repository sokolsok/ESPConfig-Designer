...
const DEVELOPMENT_RESOURCE_PLACEHOLDER: &str =
...
impl BackendConfig {
    fn from_environment(
        ...
        let backend_root = backend_override
            .clone()
            .unwrap_or_else(|| packaged_backend.unwrap_or(development_backend));
        ...
        let topology_script = packaged_script.unwrap_or(development_script);
        let default_script = backend_override
            .map(|_| backend_root.join("desktop_launcher.py"))
            .filter(|path| path.is_file())
            .unwrap_or(topology_script);
        let script = env_path("ECD_TAURI_BACKEND_SCRIPT", default_script);
    ...
    fn command(&self) -> Command {
        ...
        #[cfg(windows)]
        {
            ...
            let creation_flags = backend_creation_flags();
            if creation_flags != 0 {
                command.creation_flags(creation_flags);
            }
        }
        if !self.launcher_is_binary {
            command.arg("-I").arg("-B").arg(&self.script);
        }
        command
            .arg("--runtime-root")
    ...
    fn start(config: &BackendConfig) -> Result<Self, String> {
        if config.launcher_is_binary {
            if !config.executable.is_file() {
                return Err(format!(
                    "Backend executable does not exist: {}",
                    config.executable.display()
                ));
            }
        } else if !config.script.is_file() {
            return Err(format!(
                "Backend launcher does not exist: {}",
        ...
        loop {
            {
                ...
                {
                    return Err(format!(
                        "Backend exited before health check with status {status}"
                    ));
                }
            }
...
fn paths_overlap(first: &PathBuf, second: &PathBuf) -> Result<bool, String> {
    ...
    Ok(first_text == second_text
        || first_text.starts_with(&(second_text.clone() + "\\"))
        || second_text.starts_with(&(first_text + "\\")))
}
...
fn writable_probe(directory: &PathBuf) -> Result<(), String> {
    ...
    fs::write(&probe, b"ok\n").map_err(|error| {
        format!(
            "Workspace is not writable '{}': {error}",
            directory.display()
        )
    })?;
...
fn prepare_workspace(workspace: &PathBuf, app_data_root: &PathBuf) -> Result<PathBuf, String> {
    ...
    for directory in [
        ...
        fs::create_dir_all(&directory).map_err(|error| {
            format!(
                "Could not create workspace directory '{}': {error}",
                directory.display()
            )
        })?;
...
fn save_workspace(app_data_root: &PathBuf, workspace: &PathBuf) -> Result<(), String> {
    ...
    let contents = serde_json::to_vec_pretty(&record)
        .map_err(|error| format!("Could not encode workspace config: {error}"))?;
...
fn pick_workspace_from_dialog(app: &AppHandle, app_data_root: &PathBuf) -> Result<PathBuf, String> {
    loop {
        ...
        let selected = selection.into_path().map_err(|_| {
            "The selected workspace path is not a local filesystem path".to_string()
        })?;
...
fn packaged_resource_paths(
    ...
    String,
> {
    let explicit_root = env::var_os("ECD_TAURI_RESOURCE_ROOT").map(PathBuf::from);
    let resource_root = if let Some(value) = explicit_root.clone() {
        value
    } else {
        app.path()
            .resource_dir()
    ...
    packaged_resource_paths_from_root(&resource_root, explicit_root.is_some())
}

fn packaged_resource_paths_from_root(
    resource_root: &Path,
    required: bool,
) -> Result<
    (
...
> {
    if !resource_root.exists() {
        if required {
            return Err(format!(
                "Configured Tauri resource root does not exist: {}",
...
fn is_development_resource_placeholder(resource_root: &Path) -> bool {
    if !cfg!(debug_assertions) {
        return false;
    }
    let Ok(mut entries) = fs::read_dir(resource_root) else {
        return false;
    };
    let Some(Ok(entry)) = entries.next() else {
        return false;
    };
    if entries.next().is_some() || entry.file_name() != "README.txt" || !entry.path().is_file() {
        return false;
    }
    fs::read_to_string(entry.path())
        .map(|content| content.trim() == DEVELOPMENT_RESOURCE_PLACEHOLDER)
        .unwrap_or(false)
}
...
mod tests {
    ...
    fn workspace_preparation_creates_user_layout() {
        ...
        assert_eq!(
            prepared,
            canonical_path(&workspace).expect("canonical workspace")
        );
    ...
    fn backend_command_uses_utf8_for_python_io() {
        ...
        assert!(command
            .get_envs()
            .any(|(name, value)| name == OsStr::new("PYTHONUSERBASE") && value.is_none()));
        assert!(command
            .get_envs()
            .any(|(name, value)| name == OsStr::new("PYTHONNOUSERSITE")
    ...
    #[cfg(windows)]
    #[test]
    fn backend_console_policy_matches_the_build_profile() {
    ...
    #[test]
    fn incomplete_packaged_resources_fail_closed() {
        let root = test_root("incomplete-resources");
        fs::create_dir_all(root.join("backend")).expect("create incomplete backend");
        fs::write(root.join("backend").join("server.py"), b"").expect("write server");

        let error = packaged_resource_paths_from_root(&root, false)
            .expect_err("incomplete packaged resources must fail");
    ...
    fn development_placeholder_uses_source_resources() {
        let root = test_root("placeholder-resources");
        fs::create_dir_all(&root).expect("create placeholder root");
        fs::write(
            root.join("README.txt"),
            format!("{DEVELOPMENT_RESOURCE_PLACEHOLDER}\n"),
        )
    ...
    #[test]
    fn explicit_development_placeholder_fails_closed() {
        let root = test_root("explicit-placeholder-resources");
        fs::create_dir_all(&root).expect("create placeholder root");
        fs::write(
            root.join("README.txt"),
            format!("{DEVELOPMENT_RESOURCE_PLACEHOLDER}\n"),
        ...
        let error = packaged_resource_paths_from_root(&root, true)
            .expect_err("explicit placeholder must fail as incomplete");
