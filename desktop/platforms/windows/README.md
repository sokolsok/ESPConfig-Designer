...
- the canonical catalog from `esp-config-designer/shared/schema-catalog/`;
...
resource policy, absence of generated bytecode, the complete catalog contract,
and byte parity from canonical source to `schema-catalog/` and `web/`.
...
The canonical source is `esp-config-designer/shared/schema-catalog`, independent
from the frontend web root. The frontend build projects the same files to their
existing static URLs, while the backend reads packaged data from
`backend/schema-catalog`. The complete catalog and both projections are verified
during resource packaging. A generated SHA-256 manifest is checked again by
Tauri for full file-set completeness and by Python for every file hash before
Flask starts.
...
- package verification for every catalog JSON and reference plus projection parity;
- packaged Tauri smoke requesting catalog and schema data through static and API routes;
