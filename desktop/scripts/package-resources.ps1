...
$supplyChainSource = Join-Path $repoRoot "desktop\supply-chain"
...
$supplyChainOutput = Join-Path $OutputRoot "supply-chain"
...
foreach ($requiredPath in @(
    ...
    (Join-Path $windowsPlatformSource "python-manifest.json"),
    (Join-Path $windowsPlatformSource "requirements-bootstrap.lock"),
    (Join-Path $windowsPlatformSource "requirements-runtime.lock"),
    (Join-Path $supplyChainSource "inventory.json"),
    (Join-Path $supplyChainSource "THIRD-PARTY-NOTICES.md"),
...
New-Item -ItemType Directory -Path $supplyChainOutput -Force | Out-Null
...
foreach ($fileName in @("git-manifest.json", "python-manifest.json", "requirements-bootstrap.lock", "requirements-runtime.lock")) {
    Copy-Item -LiteralPath (Join-Path $windowsPlatformSource $fileName) -Destination (Join-Path $runtimeOutput $fileName)
}
Copy-Item -LiteralPath (Join-Path $supplyChainSource "inventory.json") -Destination (Join-Path $supplyChainOutput "inventory.json")
Copy-Item -LiteralPath (Join-Path $supplyChainSource "THIRD-PARTY-NOTICES.md") -Destination (Join-Path $supplyChainOutput "THIRD-PARTY-NOTICES.md")
