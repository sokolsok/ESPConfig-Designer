...
Assert-True ($closeExactStartupWindowBody.Contains('AddSeconds(30)') -and $closeExactStartupWindowBody -match '\$window -ne \[IntPtr\]::Zero -and\s*\(-not \$RequiredText -or \[CleanMachinePathNative\]::WindowHasChildTextContaining\(\$window, \$RequiredText\)\)') "Controlled Startup dialog wait must require both the exact top-level window and required child text before success"
Assert-True ($closeExactStartupWindowBody.Contains('$windowReady = $false') -and $closeExactStartupWindowBody -match '(?s)WindowHasChildTextContaining\(\$window, \$RequiredText\).*?\[DateTime\]::UtcNow -ge \$deadline.*?\$windowReady = \$true.*?if \(-not \$windowReady\)') "Controlled Startup dialog success must be claimed only for the complete observation inside its deadline"
...
$finalDialogTextIndex = $closeExactStartupWindowBody.LastIndexOf('[CleanMachinePathNative]::WindowHasChildTextContaining')
