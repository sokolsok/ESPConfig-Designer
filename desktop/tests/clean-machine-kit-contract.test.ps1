...
Assert-True ($orchestratorSource.Contains('Add-Type -AssemblyName UIAutomationClient') -and $orchestratorSource.Contains('Add-Type -AssemblyName UIAutomationTypes') -and $orchestratorSource.Contains('[System.Windows.Automation.TreeScope].Assembly.Location') -and $orchestratorSource.Contains('[System.Windows.Automation.ElementNotAvailableException].Assembly.Location')) "Controlled Startup dialog helper must compile against both built-in Windows UI Automation assemblies"
Assert-True ($orchestratorSource.Contains('TreeScope.Descendants') -and $orchestratorSource.Contains('Current.Name') -and $orchestratorSource.Contains('StartsWith(expectedText, StringComparison.Ordinal)') -and -not $orchestratorSource.Contains('name.IndexOf(expectedText')) "Controlled Startup dialog must require an ordinal body prefix while allowing localized OS detail"
$closeExactStartupWindowBody = [regex]::Match($orchestratorSource, '(?s)function Close-ExactStartupWindow\b.*?(?=\r?\nfunction )').Value
Assert-True ($closeExactStartupWindowBody.Contains('AddSeconds(30)') -and $closeExactStartupWindowBody -match '\$window -ne \[IntPtr\]::Zero -and\s*\(-not \$RequiredText -or \[CleanMachineUiAutomation\]::WindowHasDescendantTextContaining\(\$window, \$RequiredText\)\)') "Controlled Startup dialog wait must require both the exact top-level window and UI Automation text before success"
Assert-True ($closeExactStartupWindowBody.Contains('$windowReady = $false') -and $closeExactStartupWindowBody -match '(?s)WindowHasDescendantTextContaining\(\$window, \$RequiredText\).*?\[DateTime\]::UtcNow -ge \$deadline.*?\$windowReady = \$true.*?if \(-not \$windowReady\)') "Controlled Startup dialog success must be claimed only for the complete observation inside its deadline"
Assert-True ($closeExactStartupWindowBody -match '\$OwnedProcess\.process\.HasExited[\s\S]{0,300}FindVisibleWindow\(\[uint32\]\$OwnedProcess\.process\.Id, \$Title\) -ne \$window') "Controlled Startup dialog must revalidate the exact live process, PID, and title before closing"
$finalDialogIdentityIndex = $closeExactStartupWindowBody.LastIndexOf('[CleanMachinePathNative]::FindVisibleWindow')
$finalDialogTextIndex = $closeExactStartupWindowBody.LastIndexOf('[CleanMachineUiAutomation]::WindowHasDescendantTextContaining')
$dialogCloseIndex = $closeExactStartupWindowBody.IndexOf('[CleanMachinePathNative]::PostMessageW')
Assert-True ($finalDialogIdentityIndex -ge 0 -and $finalDialogIdentityIndex -lt $finalDialogTextIndex -and $finalDialogTextIndex -lt $dialogCloseIndex) "Controlled Startup dialog must revalidate exact identity and required text immediately before close"
