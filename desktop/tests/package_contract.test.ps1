...
$exactWindowWaitBody = [regex]::Match($simultaneousStartTestSource, '(?s)function Wait-ExactPrimaryWindow\b.*?(?=\r?\nfunction )').Value
Assert-True ($exactWindowWaitBody.Contains('AddSeconds(10)') -and $exactWindowWaitBody.Contains('FindVisibleWindowByTitle') -and $exactWindowWaitBody.Contains('$Primary.HasExited')) "Warm-launch setup must wait boundedly for the exact primary PID and title"
