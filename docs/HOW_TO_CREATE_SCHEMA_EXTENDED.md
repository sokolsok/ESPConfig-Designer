...
- `previewGroup` -> optional preview-tab grouping key for root components with different YAML domains
...
### Optional preview grouping for root components

Some root components emit different top-level YAML keys, but should still be shown together in one preview tab for UX reasons.
...
```json
{
  ...
  "platform": "",
  "renderAs": "root_map",
  "previewGroup": "wireless"
}
```

Rules:

- `previewGroup` affects preview tab grouping only
- it does not change emitted YAML
- components without `previewGroup` keep the current default grouping behavior
- prefer it only for root-only categories where multiple different YAML domains would otherwise fragment preview UX
- do not use it as a substitute for correct `domain`, `domainBy`, `domainMap`, or `renderAs`

Typical use cases:

- `Wireless Communication` root components grouped under one `Wireless` preview tab
- `Miscellaneous Components` root components grouped under one `Misc` preview tab
...
## 10a. Multi-helper orchestration

Some components require more than one helper/root component to work.

Example: a climate component may need both:

- its own root helper
- a BLE helper used by that root helper

In these cases, prefer a flat UI model at the child component level.

The user should see all required helper selectors immediately, for example:

- `Select Root hub`
- `Select BLE hub`

Do not hide one required helper inside another helper selector flow when the component depends on both.

### Modeling pattern

Use normal top-level shared helper pairs on the child component:

- local helper `object`
- matching `id_ref`
- `embedded` entry for YAML emission

If helper A must reference helper B in emitted YAML, use `embedded.injectFields` to copy selected values from the child component config into helper A payload.
  ...
  "embedded": [
    {
      ...
      "domain": "bedjet",
      "dedupeBy": "id",
      "selectLabel": "Select Root hub",
      "injectFields": [
        { "from": "ble_client_id", "to": "ble_client_id" }
      ]
    },
    {
      "key": "ble_hub",
  ...
  "fields": [
    {
      ...
      "type": "object",
      "emitYAML": "never",
      "extends": "hub_ble_client_with_tracker.json"
    },
    {
      "key": "ble_client_id",
      "type": "id_ref",
...
### `embedded.injectFields`

`injectFields` copies values from the current component config into the emitted payload of the helper.

Definition shape:

- `from` -> source field path in the current component config
- `to` -> target field path in the emitted helper payload
...
This is the preferred way to connect two required helpers while keeping the UI flat.

### Authoring rules

- if a component depends on more than one helper, prefer flat multi-helper orchestration
- do not auto-create hidden helper sections if the user should be able to reuse an existing helper
- do not duplicate a full existing helper contract inside another helper just to avoid orchestration work
- use `selectLabel` on `embedded` when the default `Select hub` is not clear enough

### What not to do

- do not force the user to open one helper only to discover that a second required helper is hidden inside it
- do not auto-emit hidden helper instances that block reuse of existing helpers
- do not merge unrelated helper contracts into one fake root helper only for UI convenience
...
- field-level `extends` on helper objects may also carry helper metadata such as `embedded`, `requirements`, and `helpUrl`
- this is important for reusable helper schemas in `base_component/`
...
- `generated_list` also works well for repeated GPIO pin lists when docs expect a normal YAML array but Add/Remove buttons are worse UX than a fixed count control
...
Example: generated primitive GPIO list
...
Use `type: "gpio"` when ESPHome expects a pin value.

Schema authors do not need a separate field type for advanced pin configuration. The runtime renderer supports both forms automatically:
...
- the saved runtime value can be either a string or an object
- Builder emits the scalar form when only the pin number is configured
- Builder emits the object form when advanced options differ from defaults
- `mode` can be emitted as a preset string or as a mapping
- `disableAdvancedConfig: true` disables the advanced GPIO toggle and advanced pin settings UI for that field while keeping normal basic pin selection enabled
- schema authors should continue to declare GPIO the same way:
...
Example with advanced UI disabled:

```json
{
  "key": "sda",
  "type": "gpio",
  "disableAdvancedConfig": true,
  "required": true,
  "placeholder": "GPIOXX"
}
```
...
Use `yamlKey` when the Builder field name should differ from the emitted YAML key.

This is useful when the schema needs a clearer UI name, but ESPHome expects a generic key such as `id`.
...
- prefer `yamlKey` only when UI clarity would otherwise be poor
- do not use it to hide unrelated YAML structure
- `emitKey: "inline"` still has separate semantics and should not be confused with `yamlKey`
...
- `dependsOn` hides a field in the UI and also suppresses normal YAML emission when the dependency is not satisfied
- the hidden value usually still remains in runtime config unless some component-specific runtime logic intentionally clears or ignores it
- for `platformByBus` schemas, the Builder still keeps the `bus` value available while deciding which dependency-gated fields emit to YAML, even though `bus` itself is not emitted
...
### Display Configurator size contract

For `display_builder: true` schemas, the Display Configurator derives canvas size from schema-backed runtime fields, not from emitted YAML text.

Preferred size sources:

- structured `dimensions.width` + `dimensions.height`
- top-level `width` + `height`
- top-level `panel_width` + `panel_height`
- `model_meta` fallback when the active schema fields do not provide explicit size

Rules:

- if size fields are behind `dependsOn`, the configurator uses them only when the dependency is currently active
- hidden size fields should not be relied on as the primary viewport contract
- use the real ESPHome field names where possible; do not invent aliases just for the configurator
- when docs do not provide an explicit size for every model, a schema may expose non-emitted helper size fields for configurator-only use, but this should be a conscious exception and documented in the schema comments/notes
